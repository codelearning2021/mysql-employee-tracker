...
function mainQuestion() {
  ...
  function addEmployee() {
    db.query(`SELECT * FROM role;`, (err, res) => {
      if (err) throw err;
      let newRoles = res.map(role => ({ name: role.title, value: role.id }));
      db.query(`SELECT * FROM employee;`, (err, res) => {
        if (err) throw err;
        let newEmployees = res.map(employee => ({ name: employee.first_name + ' ' + employee.last_name, value: employee.id }));
        inquirer.prompt([
          {
            name: 'firstName',
            type: 'input',
          ...
          db.query(`INSERT INTO employee SET ?`,
            ...
            (err, res) => {
              if (err) throw err;
              console.log(`${response.firstName} successfully added to database!`);
              mainQuestion();
            })
        })
      })
...
function updateRole() {
  db.query(`SELECT * FROM role;`, (err, res) => {
    if (err) throw err;
    let employeeRoleUpdate = res.map(role => ({ name: role.title, value: role.id }));
    db.query(`SELECT * FROM employee;`, (err, res) => {
      if (err) throw err;
      let employeeNameUpdate = res.map(employee => ({ name: employee.first_name + ' ' + employee.last_name, value: employee.id }));
      inquirer.prompt([
        {
          name: 'employee',
          type: 'rawlist',
